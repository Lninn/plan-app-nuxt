  ...
  <div class="create-form">
    ...
    <el-form ref="ruleFormRef" :rules="rules" :model="form" label-position="top">
      <el-form-item label="一级分类" prop="firstCategory">
        <el-select :loading="categoriesLoading" v-model="form.firstCategory" placeholder="请选择一级分类名称">
          <el-option
      ...
      <el-form-item label="二级分类" prop="secondCategory">
        <el-input v-model="form.secondCategory" placeholder="二级分类名称" />
      </el-form-item>
      <el-form-item>
        <el-button :loading="loading"type="primary" @click="submitForm(ruleFormRef)">创建</el-button>
        <el-button @click="navigateTo('/tasks')">取消</el-button>
      </el-form-item>
    </el-form>
  </div>
  <el-dialog v-model="firstCategoryDialogFormVisible" title="Shipping address" width="500">
    <el-form ref="firstCategoryRuleFormRef" :model="firstCategoryForm" :rules="firstCategoryFormRules">
      <el-form-item label="分类名称" prop="name" :label-width="formLabelWidth">
        <el-input v-model="firstCategoryForm.name" autocomplete="off" placeholder="分类名称" />
      </el-form-item>
    </el-form>
    <template #footer>
      <div class="dialog-footer">
        <el-button @click="firstCategoryDialogFormVisible = false">取消</el-button>
        <el-button type="primary" :loading="loading" @click="onFirstCategoryRuleFormSubmit(firstCategoryRuleFormRef)">
          确认
        </el-button>
      </div>
    </template>
